...
class BaseUploader
{
    ...
    protected function sortFiles(array $files, $sort)
    {
        ...
        usort($files, function($a, $b) use ($that, $sort) {
            ...
            if ($sort === static::SORT_FILEMTIME_ASC || $sort === static::SORT_FILEMTIME_DESC) {
                $a = filemtime($a);
                $b = filemtime($b);
            }

            if ($sort === static::SORT_FILESIZE_ASC || $sort === static::SORT_FILESIZE_DESC) {
                $a = filesize($a);
                $b = filesize($b);
            }
            ...
            if ($sort === static::SORT_FILEMTIME_ASC || $sort == static::SORT_FILESIZE_ASC) {
                return $a > $b ? -1 : 1;
            }

            if ($sort === static::SORT_FILEMTIME_DESC || $sort === static::SORT_FILESIZE_DESC) {
                return $a < $b ? -1 : 1;
            }
        });
